...
func TestOutboxStore_CreateClaimAndMarkSent(t *testing.T) {
	...
	err := store.Create(ctx, recipientEmail, outbox.Email{
		Subject:  subject,
		HTMLBody: "<b>hello</b>",
	})
...
func TestOutboxStore_ClaimNextPending_ReturnsOldestPendingMessage(t *testing.T) {
	...
	err := store.Create(ctx, firstRecipient, outbox.Email{Subject: firstSubject, HTMLBody: "1"})
	require.NoError(t, err)
	err = store.Create(ctx, secondRecipient, outbox.Email{Subject: secondSubject, HTMLBody: "2"})
	require.NoError(t, err)
...
func TestOutboxStore_Create_UsesTransaction(t *testing.T) {
	...
	err := store.Create(dbtx.WithTransactionContext(context.Background(), tx), recipientEmail, outbox.Email{
		Subject:  subject,
		HTMLBody: "body",
	})
	require.NoError(t, err)
...
func claimPendingTestMessage(t *testing.T, ctx context.Context, store *OutboxStore, recipientEmail string, subject string) outbox.Email {
	t.Helper()

	var foreignItems []outbox.Email
...
func releaseClaimedOutboxItems(t *testing.T, ctx context.Context, store *OutboxStore, items []outbox.Email) {
	t.Helper()
...
func requireNoPendingOutboxRows(t *testing.T, db queryExecutor, recipientEmail string, subject string) {
	t.Helper()
