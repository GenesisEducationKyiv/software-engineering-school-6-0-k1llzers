...
func (s *MessageInboxStore) ClaimForProcessing(ctx context.Context, messageID uuid.UUID, messageType string, payloadJSON json.RawMessage) (bool, error) {
	insertQuery := `
	...
	result, err = appdb.NewQueryExecutor(ctx, s.db).ExecContext(ctx, updateQuery, messageID)
	if err != nil {
		return false, err
	}
